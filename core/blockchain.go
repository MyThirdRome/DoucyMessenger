...
func NewBlockchain(storage *storage.LevelDBStorage, config *utils.Config) (*Blockchain, error) {
	...
	if err == nil && rewards != nil {
		if rewardsMap, ok := rewards.(map[string]map[string]float64); ok {
			bc.pendingRewards = rewardsMap
		}
	}
...
func (bc *Blockchain) AddWallet(w *wallet.Wallet) error {
	...
	bc.wallets[w.GetAddress()] = w
	return bc.storage.SaveWallet(w.GetAddress(), []byte(w.GetPrivateKeyString()))
}
...
func (bc *Blockchain) ProcessMessageResponse(response *messaging.Message) error {
	...
	if reward, exists := bc.pendingRewards[response.Receiver][response.Sender]; exists {
...
func (bc *Blockchain) WhitelistAddress(from, to string) error {
	...
	// Save to storage
	return bc.storage.SaveWhitelist(from, []string{to})
}
...
func (bc *Blockchain) JoinGroup(address, groupID string) error {
	...
	groupObj, ok := group.(*messaging.Group)
	if !ok {
		return errors.New("invalid group object")
	}

	if !groupObj.IsPublic && groupObj.Owner != address {
...
func (bc *Blockchain) distributeValidatorRewards(amount float64) error {
	...
	activeValidators := 0
	for _, v := range bc.validators {
		if v.IsEligible(bc.validatorMinDeposit) {
			activeValidators++
		}
	}
